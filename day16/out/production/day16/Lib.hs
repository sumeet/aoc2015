...
-- badboy from https://hackage.haskell.org/package/numeric-prelude-0.4.3.2/docs/src/Number.Positional.html#sliceVertPair
pairs :: [a] -> [(a, a)]
pairs (x0 : x1 : xs) = (x0, x1) : pairs xs
pairs [] = []
pairs _ = error "odd number of elements"

parseSue :: String -> Map String Int
parseSue line = fromList $ map (second read) $ pairs $ drop 2 $ words $ filter (`notElem` ",:") line

run :: IO ()
run = print $ map ((+ 1) . fst) $ filter ((`matchesAll` clues) . snd) $ zip [0 ..] sues
  where
    sues = map parseSue $ lines input
    clues = map parseClue $ lines cluesString

matchesAll :: Map String Int -> [(String, Int)] -> Bool
matchesAll sue = all (match sue)

match :: Map String Int -> (String, Int) -> Bool
match sue (clueName, clueValue) =
  (clueName `notMember` sue)
    || ( case clueName of
           "trees" -> clueValue < sueValue
           "cats" -> clueValue < sueValue
           "pomeranians" -> clueValue > sueValue
           "goldfish" -> clueValue > sueValue
           _ -> clueValue == sueValue
       )
  where
    sueValue = sue ! clueName

parseClue :: String -> (String, Int)
parseClue s = (head ws, read $ ws !! 1) where ws = words $ filter (/= ':') s
...
input :: String
input =
  [r|Sue 1: goldfish: 6, trees: 9, akitas: 0

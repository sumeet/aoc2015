...
let has_prefix needle haystack = Str.string_match (Str.regexp needle) haystack 0

let strip_prefix needle haystack =
  let start = (String.length needle) in
  let last = String.length haystack - String.length needle in
  String.sub haystack start last

let parse_point s = match Str.split (Str.regexp ",") s with
  | [x; y] -> (int_of_string(x), int_of_string(y))
  | _ -> failwith "couldn't parse point"

let parse_range s = match Str.split (Str.regexp " through ") s with
  | [x; y] -> parse_point x, parse_point y
  | _ -> failwith ("unable to parse " ^ s)

let parse_instruction s : instruction =
  if has_prefix "turn on" s then
    let rangestring = strip_prefix "turn on " s in
    (TurnOn, (parse_range rangestring))
  else if has_prefix "turn off" s then
    let rangestring = strip_prefix "turn off " s in
    (TurnOff, (parse_range rangestring))
  else
    let rangestring = strip_prefix "toggle " s in
    (Toggle, (parse_range rangestring))


let instructions = String.split_on_char '\n' input
                   |> List.map parse_instruction

let count_lights lights =
  let count = ref 0 in
  for i = 0 to (Array.length lights) - 1 do
    for j = 0 to (Array.length lights.(i)) - 1 do
      count := !count + if lights.(i).(j) then 1 else 0
    done
  done;
  !count

let apply action x y lights = match action with
  | TurnOn -> true
  | TurnOff -> false
  | Toggle -> not lights.(x).(y)

let () =
  let all_lights = Array.make_matrix 1000 1000 false in
  List.iter (fun (action, ((lo_x, lo_y), (hi_x, hi_y))) ->
      for x = lo_x to hi_x do
        for y = lo_y to hi_y do
          all_lights.(x).(y) <- apply action x y all_lights
        done
      done
    ) instructions;
  Format.printf "%d\n" (count_lights all_lights)
